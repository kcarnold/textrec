...
confints %>%
  subset(type == "means") %>% 
  subset(measure != "frac_predictable") %>% 
  ...
  geom_text(aes(label=sprintf("%1.2f", conf_low), y=conf_low), hjust="right", size=3) +
  geom_text(aes(label=sprintf("%1.2f", conf_high), y=conf_high), hjust="left", size=3) +
  expand_limits(x=0) +
...
multiBar <- function(measures.to.vis, nrow=3) {
  blockLevelMinusMean %>%
    ...
    ggplot(aes(x=condition, y=value)) +
      ...
      labs(x="Suggestion Visibility", y="") +
      coord_flip() +
      theme(
