...
confints %>%
  subset(type == "means") %>% 
  subset(measure %in% list("num_predictable", "num_unpredictable", "num_words")) %>% 
...
```{r bootstrap_pairwise, fig.width=7, fig.height=7}
...
cur_measures <- list("num_predictable", "num_unpredictable", "frac_predictable", "num_words")
...
confints %>%
  ...
  geom_text(aes(label=sprintf("%1.2f", conf_low), y=conf_low), hjust="right", vjust="bottom", size=3) +
  geom_text(aes(label=sprintf("%1.2f", conf_high), y=conf_high), hjust="left", vjust="bottom", size=3) +
  #expand_limits(x=0, y=c(-1.5, 2)) +
  geom_blank(
    aes(y=value), 
    data=ranges %>% pivot_longer(-measure),#data.frame(measure="frac_predictable", lims=c(-10, 10)),
  inherit.aes = F) +
