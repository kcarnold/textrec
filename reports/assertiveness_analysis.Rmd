...
analyze_one_iteration(c("num_predictable"))
...
if (do_compute) {
  ...
  boot_results <- readRDS("bootstrap_results_20191005.rds")
}
```
...
```{r perBlockBootstrap}
blockLevelSubjective <- data %>% 
  group_by(participant, condition, block) %>% summarise_at(c("TLX_sum", "physical", "mental"), first) %>% 
...
analyze_one_iteration <- function(measures, data, formulaSuffix=" ~ condition + (1|participant)") {
  pairs_to_stats <- function(emmgrid) {
    emmgrid %>% 
      pairs() %>% 
      as.data.frame(infer=F) %>% 
...
#analyze_one_iteration(c("TLX_sum", "physical", "mental"), blockLevelSubjective)

analyze_n_iterations <- function(dataName, n_iterations, measures, formulaSuffix) {
  data <- get(dataName)
  lapply(1:n_iterations, function(x) analyze_one_iteration(measures, data, formulaSuffix = formulaSuffix)) %>% bind_rows()
}
...
if (do_compute) {
  ...
  measures_to_analyze = c("TLX_sum", "physical", "mental")
  
  clusterExport(cluster, varlist = c("getBootstrapSample", "analyze_one_iteration", "analyze_n_iterations", "blockLevelSubjective"))
  clusterCall(cluster, doAnalysisSetup);
  ...
  system.time(
    boot_results <- pbapply::pblapply(
      1:total_iterations,
      function(i, measures, data=blockLevelSubjective) {
  ...
  saveRDS(boot_results, "TLX_bootstraps.rds");
  
  stopCluster(cluster);
} else {
  boot_results <- readRDS("TLX_bootstraps.rds")
}
