...
function getClosestKey(keyRects, touchX, touchY) {
  ...
  keyRects.forEach(function(krect) {
    var rect = krect.rect,
      ...
      x = rect.left + hwidth,
      y = rect.top + hheight;
    var dx = Math.max(0, Math.abs(touchX - x) - hwidth),
      dy = Math.max(0, Math.abs(touchY - y) - hheight);
...
export class Keyboard extends React.Component<{ useElectricDelete: string }> {
  ...
  handleTouchStart = evt => {
    ...
    let { clientX, clientY } =
      evt.type === "touchstart" ? evt.changedTouches[0] : evt;
