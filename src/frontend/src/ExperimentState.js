...
export class ExperimentStateStore {
  constructor(condition, sugFlags) {
    ...
    M.extendObservable(this, {
      ...
      get visibleSuggestions() {
        ...
        let serverIsValid =
          this.condition.showPredictions === false ||
          fromServer.request_id === this.contextSequenceNum;
        if (!serverIsValid) {
        ...
        ["predictions", "synonyms"].forEach(type => {
          if (
            type === "predictions" &&
            this.condition.hideFullwordPredictions &&
            !this.hasPartialWord
          ) {
            result[type] = [];
          } else {
        ...
        if (attentionCheck !== null && serverIsValid) {
          ...
          if (
            attentionCheckType === "predictions" ||
            attentionCheckType === "synonyms"
          ) {
      ...
      get showSynonyms() {
        if (this.condition.showSynonyms) {
          if (this.condition.showSynonymsXorPredictions) {
            return (
              this.curText.length > 0 &&
              this.suggestionContext.curWord.length === 0
            );
          }
          return true;
        }
      ...
      get showPredictions() {
        if (
          this.condition.dontRequestSuggestions ||
          !this.condition.showPredictions
        ) {
          return false;
        }
        if (
          this.condition.showSynonyms &&
          this.condition.showSynonymsXorPredictions
        ) {
      ...
      tapKey: M.action(event => {
        ...
        if (
          this.condition.showRelevanceHints &&
          !isNonWord &&
  ...
  validateAttnCheck(event) {
    ...
    if (attentionCheckType === "text") {
      ...
      passed =
        attentionCheckType === event.which &&
        this.attentionCheck.slot === event.slot;
    }
  ...
  handleEvent = event => {
    ...
    if (this.curText !== prevState.curText) {
      ...
      if (
        this.condition.useAttentionCheck &&
        rng() < this.condition.useAttentionCheck
      ) {
        let ac = {};
        let choices = ["text"];
        ...
        let numSlots =
          ac.type === "predictions"
            ? 3
            : this.condition.sugFlags.num_alternatives;
    ...
    if (
      !this.condition.dontRequestSuggestions &&
      this.lastSuggestionsFromServer.request_id !== this.contextSequenceNum
    ) {
