...
export const ControlledStarRating = inject("dispatch", "state")(
  observer(({ state, dispatch, name }) => (
    <div>
      <StarRatingComponent
        name={name}
        starCount={5}
        value={state.controlledInputs.get(name) || 0}
        onStarClick={value => {
          dispatch({ type: "controlledInputChanged", name, value });
        }}
        renderStarIcon={(idx, value) => (
