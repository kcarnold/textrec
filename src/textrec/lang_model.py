...
def encode_bigrams(bigrams, model):
    ...
    filtered_bigrams = {
        a: pull_2nd(heapq.nlargest(100, nexts)) for a, nexts in encoded_bigrams.items()
    }
...
class Model:
    ...
    def _load(self):
        ...
        self.unfiltered_bigrams, self.filtered_bigrams = encode_bigrams(
            bigrams, self.model
        )

        # Vocab trie
        self.vocab_trie = datrie.BaseTrie(
            set(itertools.chain.from_iterable(self.id2str))
        )
    ...
    def prune_bigrams(self):
        ...
        while True:
            new_bigrams = {
                k: [tok for tok in v if len(bigrams.get(tok, [])) > 0]
    ...
    def _compute_pos(self):
        ...
        pos_tags = [
            nltk.pos_tag([w or "UNK"], tagset="universal")[0][1] for w in self.id2str
        ]
    ...
    def word_lengths(self):
        if not hasattr(self, "_word_lengths"):
            self._word_lengths = np.array(
                [len(w) if w is not None else 0 for w in self.id2str]
            )
    ...
    def score_seq(self, state, words):
        ...
        for word in words:
            ...
            score += self.model.base_score_from_idx(
                state, self.model.vocab_index(word), new_state
            )
            state = new_state
    ...
    def score_seq_by_word(self, state, words):
        ...
        for word in words:
            ...
            scores.append(
                LOG10
                * self.model.base_score_from_idx(
                    state, self.model.vocab_index(word), new_state
                )
            )
            state = new_state
    ...
    def advance_state(self, state, tok):
        ...
        return (
            ...
            LOG10
            * self.model.base_score_from_idx(
                state, self.model.vocab_index(tok), new_state
            ),
        )
    ...
    def next_word_logprobs_raw(self, state, prev_word, prefix_logprobs=None):
        ...
        if prefix_logprobs is not None:
            ...
            next_words = [
                w for w in next_words if w != self.eos_idx and w != self.eop_idx
            ]
        if len(next_words) == 0:
            return [], np.zeros(0)
    ...
    def eval_logprobs_for_words(self, state, next_words):
        ...
        for next_idx, word_idx in enumerate(next_words):
            logprobs[next_idx] = self.model.base_score_from_idx(
                state, word_idx, new_state
            )
        logprobs *= LOG10
        return logprobs
...
def make_model(model_name, order=5, prune=2):
    ...
    with open(in_file_name, "rb") as in_file, open(arpa_file_name, "wb") as arpa_file:
        ...
        subprocess.run(lmplz_cmd, stdin=in_file, stdout=arpa_file, check=True)
...
BeamEntry = namedtuple(
    "BeamEntry",
    "score, words, done, penultimate_state, last_word_idx, num_chars, extra",
)
...
def beam_search_phrases_init(model, start_words, **kw):
    ...
    return [
        (0.0, [], False, start_state, model.model.vocab_index(start_words[-1]), 0, None)
    ]


def beam_search_phrases_extend(
    model,
    beam,
    *,
    iteration_num,
    ...
    for entry in beam:
        ...
        if done:
            ...
            if iteration_num > 0:
                ...
                model.model.base_score_from_idx(
                    penultimate_state, last_word_idx, last_state
                )
            else:
            ...
            for next_idx, word_idx in enumerate(next_words):
                ...
                if word in words:
                    ...
                    bonus = bonus_words.get(word, 0.0)

                main_model_score = LOG10 * model.model.base_score_from_idx(
                    last_state, word_idx, new_state
                )
...
def beam_search_phrases_loop(
    model, beam, *, length_after_first, prefix_logprobs=None, start_idx=0, **kw
):
    for iteration_num in range(start_idx, length_after_first):
        beam = beam_search_phrases_extend(
            model,
            beam,
